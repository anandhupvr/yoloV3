...
class Arch:
	...
	def darknet(self):
		conv1 = tf.layers.conv2d(self.x, filters=32, kernel_size=[3, 3], padding="same", activation=tf.nn.leaky_relu, name="conv_1")
		conv1 = tf.layers.batch_normalization(conv1)
		maxpool1 = tf.nn.max_pool(conv1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding="SAME")
		conv2 = tf.layers.conv2d(maxpool1, filters=64, kernel_size=[3, 3], padding="same", activation=tf.nn.leaky_relu, name="conv_2")
